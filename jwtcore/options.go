...
// An Option configures a TokenManagement.
type Option[T jwt.Claims, PT Claims[T]] interface {
	apply(*TokenManagement[T, PT])
}

// optionFunc wraps a func, so it satisfies the Option interface.
type optionFunc[T jwt.Claims, PT Claims[T]] func(*TokenManagement[T, PT])

func (f optionFunc[T, PT]) apply(manager *TokenManagement[T, PT]) {
	f(manager)
}

// WithDecryptKey 设置解密密钥.
func WithDecryptKey[T jwt.Claims, PT Claims[T]](key string) Option[T, PT] {
	return optionFunc[T, PT](func(t *TokenManagement[T, PT]) {
		t.DecryptKey = key
	})
}
